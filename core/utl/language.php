...
{
    /**
     * @var array ISO-639-2 alpha-3 list of languages
     */
    private static $Alpha3;

    /**
     * @var array ISO-639-2 alpha-3 list of languages with "B" and "T"
     */
    private static $Alpha3Full;

    /**
     * @var array ISO-639-1 alpha-2 list of languages
     */
    private static $Alpha2;
    ...
    public static function getAlpha3()
    {
        self::loadDatabase();
        return self::$Alpha3;
    }

    public static function getAlpha3Full()
    {
        self::loadDatabase();
        return self::$Alpha3Full;
    }

    private static function loadDatabase()
    {
        if (!empty(self::$Alpha2)) {
            // already loaded
            return true;
        }
        // ISO 639-2
...
EOL;
        foreach(explode("\n", $str) as $line) {
            ...
            if (!empty($codes[2])) {
                self::$Alpha2[$codes[2]] = $name;
                self::$Alpha3[$codes[0]] = $name;
            }
            if (!empty($codes[0])) {
                self::$Alpha3Full[$codes[0]] = $name;
            }
        }
    }
